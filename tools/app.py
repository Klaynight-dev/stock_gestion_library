...
from tools.menu_setup import create_menu
log_action(f"Importation du fichier 'menu_setup.py'", success=True)
...
#         books_layout.addWidget(self.text_browser)
    ...
    def supprimer_logs(self):
        
        dossier= "data/logs/"
        
        # Vérifier si le dossier existe
        if not os.path.exists(dossier):
            log_action(f"Le dossier '{dossier}' n'existe pas.", False, "Dossier introuvable")
            QMessageBox.warning(self, "Dossier introuvable","Le dossier '{dossier}' n'existe pas.")
            return
        ...
        for fichier in fichiers:
            # Vérifier si le fichier est un fichier .log
            if fichier.endswith(".log"):
                chemin_fichier = os.path.join(dossier, fichier)
                # Supprimer le fichier
                os.remove(chemin_fichier)
                log_action(f"Fichier '{fichier}' supprimé avec succès.", True)
        QMessageBox.information(self, "Tout les fichiers ont été supprimé avec succès", "Le cache à bien été vidé")
...
#         print(self.tab_widget.currentIndex())
    ...
    def update_book_table(self, books=None):
        self.update_table(self.book_table, books, ['book_id', 'title', 'author', 'publisher', 'isbn', 'total_copies', 'available_copies'])

    def update_user_table(self, users=None):
        self.update_table(self.user_table, users, ['user_id', 'fname', 'name', 'mail', 'take'])
        
    def update_table(self, table_widget, items=None, column_names=None):
        """
        Met à jour un QTreeWidget avec une liste d'items en fonction des noms de colonnes spécifiés.

        :param table_widget: QTreeWidget à mettre à jour
        :param items: Liste des éléments à afficher dans le tableau
        :param column_names: Liste des noms de colonnes pour l'affichage
        """
        # Efface le contenu actuel du tableau
        table_widget.clear()

        # Détermine les éléments à afficher si non fournis
        if not items:
            if 'book_id' in column_names:
                items = self.library.display_books()
            elif 'user_id' in column_names:
                items = self.user_gestion.display_users()
        
        # Mise à jour du tableau en fonction des colonnes
        for item in items:
            tree_item = QTreeWidgetItem(table_widget)

            # Associe les données aux colonnes de manière récursive
            for col, column_name in enumerate(column_names):
                # Utilise getattr pour obtenir la valeur de l'attribut
                value = getattr(item, column_name, '')
                # Gère les cas où la valeur peut être de type non chaîne (par exemple, un nombre)
                tree_item.setText(col, str(value))
